...
export const SYSTEM_COMPONENTS: SystemComponent[] = [
  // Networking
  {
    ...
    stateful: false,
    description:
      "End users or client applications that generate requests — browsers, mobile apps, IoT devices, or backend callers. Every system design starts here: traffic enters through DNS, CDN, or an API gateway. This node is the simulation entry point for load testing your architecture.",
  },
  {
    id: "dns",
    label: "DNS",
  ...
  // AI / ML Infrastructure
  {
    ...
    stateful: false,
    description:
      "Unified API layer for large language model inference — handles routing across model providers (OpenAI, Anthropic, self-hosted), prompt templating, token budgeting, response streaming (SSE), caching of identical prompts, and fallback when a provider is down. Critical for any AI product that needs provider abstraction, cost controls, and observability over LLM calls. Examples include LiteLLM, OpenRouter, and internal gateways at Notion, Intercom, and Shopify.",
  },
  {
    ...
    stateful: false,
    description:
      "Converts text, images, or documents into dense vector embeddings for semantic search, clustering, and RAG pipelines. Typically wraps an embedding model (text-embedding-3, Cohere, BGE) behind a batching API with GPU autoscaling. Used during document ingestion (offline) and query-time encoding (online). OpenAI Embeddings API, Cohere Embed, and self-hosted sentence-transformers on Triton/vLLM are common implementations.",
  },
];
